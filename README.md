...
- **Nivel 1 (incluido, obligatorio):** `python scripts/validate_contracts.py knowledge/contracts` (incluye el sello obligatorio `tests_sha256` del oráculo congelado — ver [`knowledge/validacion.md`](knowledge/validacion.md)) + `python scripts/validate_specs.py specs` + `python scripts/lint_ascii.py scripts` + el `test_command` del contrato, todo en verde local y en CI (`.github/workflows/validate.yml`, matriz dual-OS). Ningún contrato se considera terminado hasta pasar el nivel 1.
